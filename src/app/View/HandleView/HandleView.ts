...
class HandleView extends Observer {
  ...
  positionOptions: { clientX: number, clientY: number, offsetLeft: number, offsetTop: number, offsetHeight: number};
  ...
  private handleWindowMouseMove = (event: MouseEvent) => {
    ...
    const { clientX,
      ...
      offsetLeft,
      offsetTop,
      offsetHeight } = this.positionOptions;
  ...
  private handleRunnerMouseDown = (event: MouseEvent) => {
    ...
    if (target && target instanceof HTMLElement) {
      ...
      this.positionOptions = { clientY, clientX, offsetLeft, offsetTop, offsetHeight };
